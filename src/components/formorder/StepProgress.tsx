            ...
            <span
              className={`z-10 grid size-8 place-items-center rounded-full border-2 ${
                isDone
                  ...
                  : isCurrent
                    ? "border-[#dd5f83] bg-white text-[#dd5f83] dark:bg-[#202a23]"
                    : "border-stone-200 bg-[#faf9f7] text-stone-300 dark:border-white/15 dark:bg-[#161d18] dark:text-stone-500"
              }`}
            >
              {isDone ? <Check size={15} /> : index + 1}
            </span>
            <span className={isCurrent ? "font-semibold text-stone-800 dark:text-stone-100" : "text-stone-400"}>
              {title}
            </span>
            {index < steps.length - 1 && (
              <span
                className={`absolute left-1/2 top-4 h-0.5 w-full ${
                  isDone ? "bg-[#dd5f83]" : "bg-stone-200 dark:bg-white/15"
                }`}
              />
            )}
          </Wrapper>
